...
class SaleOrder(models.Model):
    ...
    def sync_so_goflow(self, lastcall):
        ...
        def split_list(orders, wanted_parts=1):
            length = len(orders)
            return [orders[i * length // wanted_parts: (i + 1) * length // wanted_parts] for i in range(wanted_parts)]

            # data is an object or list

        if orders and len(orders) > 10:
            A, B, C, D = split_list(orders, wanted_parts=4)
            A_calculation = threading.Thread(target=self.loop_records, args=(A, company_for_glow))
            B_calculation = threading.Thread(target=self.loop_records, args=(B, company_for_glow))
            C_calculation = threading.Thread(target=self.loop_records, args=(C, company_for_glow))
            D_calculation = threading.Thread(target=self.loop_records, args=(D, company_for_glow))
            A_calculation.start()
            B_calculation.start()
            C_calculation.start()
            D_calculation.start()

    def loop_records(self, orders, company_for_glow):
        with odoo.registry(self.env.cr.dbname).cursor() as new_cr:
            new_env = odoo.api.Environment(new_cr, odoo.SUPERUSER_ID, self.env.context)
            for order in orders:
                goflow_store_id = order["store"]["id"]
                goflow_store_obj = new_env['goflow.store'].search([('goflow_id', '=', goflow_store_id)], limit=1)
                vals_partner_ship = {}
                vals_partner_ship['name'] = (order["shipping_address"]["first_name"] or '') + (
                ...
                ship_country_id = new_env['res.country'].search([('code', '=', partner_ship_country_code.upper())],
                                                                limit=1)
                if ship_country_id:
                    vals_partner_ship['country_id'] = ship_country_id.id
                    ship_state_id = new_env['res.country.state'].search([
                ...
                partner_ship_obj = new_env['res.partner'].search([('name', '=', vals_partner_ship['name'])], limit=1)

                if not partner_ship_obj:
                    vals_partner_ship['parent_id'] = goflow_store_obj.partner_id.id
                    partner_ship_obj = new_env['res.partner'].create(vals_partner_ship)
                ...
                warehouse_obj = new_env['stock.warehouse'].search([('goflow_id', '=', goflow_warehouse_id)], limit=1)
                if not warehouse_obj:
                    warehouse_obj = new_env['stock.warehouse'].create(
                ...
                check_if_order_exists = new_env['sale.order'].search([('goflow_id', '=', goflow_id)], limit=1)
                ...
                if check_if_order_exists:
                    ...
                    for line in tracking_line_list:
                        ...
                        line_obj = new_env['sale.order.line'].search([('goflow_id', '=', goflow_line_id)], limit=1)
                        if line_obj:
                ...
                if not check_if_order_exists:
                    ...
                    order_vals[
                        'partner_shipping_id'] = partner_ship_obj and partner_ship_obj.id or goflow_store_obj.partner_id.id
                    ...
                    so = new_env['sale.order'].create(order_vals)
                    for line in order_lines:
                        ...
                        product_obj = new_env["product.product"].search([('goflow_id', '=', goflow_product_id)],
                                                                        limit=1)
                        if not product_obj:
                            product_obj = new_env['product.product'].create(
                                {'name': goflow_product_name, 'goflow_id': goflow_product_id,
                                 'goflow_item_no': goflow_product_item_no,
                        ...
                        tracking_obj = next(
                            filter(lambda d: d.get('order_line_id') == goflow_line_id, tracking_line_list), None)
                        try:
                            tracking_number = tracking_obj['tracking_number']
                        except:
                            tracking_number = ''
                        new_env['sale.order.line'].create({
